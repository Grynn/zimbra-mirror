...
function SoundManager(smURL,smID) {
  ...
  this.getSoundById = function(sID,suppressDebug) {
    ...
    if (!result && !suppressDebug) {
      soundManager._writeDebug('"'+sID+'" is an invalid sound ID.');
  ...
  this._failSafely = function() {
    ...
    if (!self._disabled) {
      self._writeDebug('soundManager.createSound(): JS-&gt;Flash communication failed (security restrictions, or movie missing)');
      ...
      if (self._didAppend) self._writeDebug('Note: Movie added via JS method, static object/embed in-page markup method may work instead.');
      self.disable();
    }
  }
  ...
  this._createMovie = function(smID,smURL) {
    ...
    var debugHTML = '<div id="soundmanager-debug"></div>';
    if (useDOM) {
    ...
    self._writeDebug('soundManager._createMovie(): trying to load <a href="'+smURL+'">'+smURL+'</a>');
  }
  ...
  this.initComplete = function() {
    ...
    if (typeof soundManagerLoad != 'undefined') {
      try {
        ...
        self._writeDebug('soundManagerLoad() threw an exception: '+e.message);
        throw e; // wheeee! (ensure normal browser console gets message)
      }
    }
  }
...
function SMSound(oSM,oOptions) {
  ...
  this._onload = function(bSuccess) {
    ...
    sm._writeDebug('SMSound._onload(): "'+self.sID+'"'+(bSuccess?' loaded.':' failed to load (possibly cached?)'));
...
var soundManager = new SoundManager('../public/SoundPlayer/soundmanager2.swf');
