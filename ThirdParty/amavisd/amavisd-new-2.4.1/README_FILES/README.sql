...
--       it is may be more useful to declare is as:
--         time_iso TIMESTAMP,
--       and set $timestamp_fmt_mysql=1 in amavisd.conf to avoid MySQL
--       inability to accept ISO 8601 timestamps with zone Z and delimiter T
...
--   - instead of declaring a time_iso field in table msgs as a string:
--         time_iso char(16) NOT NULL,
--       it is more useful to declare is as:
--         time_iso TIMESTAMP WITHOUT TIME ZONE NOT NULL,
...
--   but a change in the program is required: edit: $timestamp_fmt_mysql=1
CREATE TABLE msgs (
...
-- NOTE: time_num field was introduced to this table with version 2.4.0
...
SELECT
  ...
  WHERE content IS NOT NULL AND UNIX_TIMESTAMP()-msgs.time_num < 100
...
An example of a log/report/quarantine database housekeeping;
...
--  select records based on ISO 8601 timestamps. This is particularly suitable
--  for PostgreSQL:
--DELETE FROM msgs WHERE time_iso < now() - interval '7 days';
...
--  (optional) optimize tables from time to time
