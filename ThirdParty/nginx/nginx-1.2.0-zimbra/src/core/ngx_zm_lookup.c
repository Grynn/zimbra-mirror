...
ngx_zm_lookup_send_request_handler(ngx_event_t *wev)
{
    ...
    if (wev->timedout) {
        ngx_log_error(NGX_LOG_ERR, wev->log, NGX_ETIMEDOUT,
            "zm lookup: route handler timed out");
        work->result = ZM_LOOKUP_WRITE_TIMEOUT;
        work->err = ERRMSG[ZM_LOOKUP_WRITE_TIMEOUT];
        ngx_zm_lookup_close_connection(ctx);
        work->on_failure(work);
        return;
    }
    ...
    n = ngx_send(c, ctx->lookup_req->pos, size);
...
ngx_zm_lookup_recv_response_handler(ngx_event_t *rev)
{
    ...
    n = ngx_recv(c, ctx->lookup_resp->pos, size);
