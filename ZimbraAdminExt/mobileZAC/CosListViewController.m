...
@implementation CosListViewController
...
-(CosListViewController *) initWithTitle: (NSString *)title
{
	self = [super init];
	self.title = title;
	//[self.tabBarItem initWithTitle:self.title image:createImage(((float) 75.5f / 10.0f)) tag:0];
	UIImage *tabIcon = [UIImage imageNamed:@"cos.png"];
	...
	self.fieldList = [NSMutableArray arrayWithObjects:@"owner", @"cn", @"description", nil];
...
{
	UIImage *img_account_blue = [UIImage imageNamed:@"cositem.png"];
	UIImage *img_account_red = [UIImage imageNamed:@"cositem_green.png"];
	CGSize sz = CGSizeMake(32,32);
...
- (void)loadView
{
	...
	NSString * kGetURLText = @"http://10.117.4.35:8080/mytest/zimbra-cluster-cosinfo.txt";
	...
	NSString * kPutCmdFile = @"zimbra-cluster-cmds-req-delcos.txt";
	...
	search.placeholder = @"Search Class of Service";
