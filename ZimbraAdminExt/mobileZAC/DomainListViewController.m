...
-(DomainListViewController *) initWithTitle: (NSString *)title
{
	self = [super init];
	self.title = title;
	//[self.tabBarItem initWithTitle:self.title image:createImage(((float) 75.5f / 10.0f)) tag:0];
	UIImage *tabIcon = [UIImage imageNamed:@"dom.png"];
	...
	self.fieldList = [NSMutableArray arrayWithObjects:@"owner", @"zimbradomainname", @"zimbradomainstatus",nil];
...
{
	UIImage *img_account_blue = [UIImage imageNamed:@"dom.png"];
	...
	if (acctInfo == nil || [acctInfo count] < 1) {
		...
		NSString *domainname = [[acctInfo objectAtIndex:1] copy];
		if (domainname && [domainname length] > 0) {
			cell.text = domainname;
			cell.detailTextLabel.text = curAcct;
		} else {
			cell.text = [[curAcct componentsSeparatedByString:@"@"] objectAtIndex:0];
			cell.detailTextLabel.text = curAcct;
		}
		NSString *status = [acctInfo objectAtIndex:2];
...
- (void)loadView
{
	...
	NSString * kGetURLText = @"http://10.117.4.35:8080/mytest/zimbra-cluster-domaininfo.txt";
