...
<c:if test="${fn:length(tags) eq 1}"><app:img src="${tags[0].miniImage}" alt="${tagNames}" title="${tagNames}"/></c:if>
<c:if test="${fn:length(tags) gt 1}"><app:img src="tags/MiniTagStack.gif" alt="${tagNames}" title="${tagNames}"/></c:if>
