...
ECHO symstore add /f %BASE%\*\rtl\*.* /s \\%INDEX_HOST%\Zbuild3\symbols\Migration /t "Zimbra Migration Tools Modules" /v "%BUILD_VERSION%" /c "%BUILD_DATE%"
symstore add /f %BASE%\Win32\rtl\*.* /s \\%INDEX_HOST%\Zbuild3\symbols\Migration /t "Zimbra Migration Tools x86 Modules" /v "%BUILD_VERSION%" /c "%BUILD_DATE%"
IF ERRORLEVEL 1 exit /B 1 
