...
MAPIAccessAPI::MAPIAccessAPI(wstring strExchangeHostName, wstring strAdminProfileName, wstring strUserName):
	m_zmmapisession(NULL),m_userStore(NULL),m_defaultStore(NULL),m_rootFolder(NULL)
{
...
HRESULT MAPIAccessAPI::OpenSessionAndStore()
{
	...
	hr=Zimbra::MAPI::Util::GetUserDnAndServerDnFromProfile(m_zmmapisession->GetMAPISessionObject(), 
	...
	Zimbra::MAPI::Util::GetUserDN(m_strExchangeHostName.c_str(),m_strUserName.c_str(),wstruserdn);
	hr=m_zmmapisession->OpenOtherStore(m_defaultStore->GetInternalMAPIStore(),pwstrExchangeServerDN, 
...
bool MAPIAccessAPI::iterate_folders(Zimbra::MAPI::MAPIFolder &folder, tree<Folder_Data> &tr, tree<Folder_Data>::iterator tritr)
{
...
HRESULT MAPIAccessAPI::IterateTree(tree<Folder_Data> &tr)
{
	...
	{      
		...
		while(sib2!=end2) 
		{
			...
			cout << "   ";
			wcout << (LPTSTR)(*sib2).name.c_str();
			cout << " lpb Lenght " << (*sib2).sbin.cb << endl;
