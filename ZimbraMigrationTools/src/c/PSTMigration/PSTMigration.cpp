...
void ZCFileUploadTest()
{
    ...
    Zimbra::Rpc::ZimbraConnection *z_pConnection = new Zimbra::Rpc::ZimbraConnection(
        L"migration", lpServerAddress, nPort,
        false, 0, L"");
...
void CreateExchangeMailBox()
{
    ...
    try
    {
        ...
        exchadmin->CreateExchangeMailBox(Zimbra::MAPI::DEFAULT_ADMIN_MAILBOX_NAME,
            Zimbra::MAPI::DEFAULT_ADMIN_PASSWORD,
            L"Administrator",
            L"z1mbr4Migration");
        ...
        for (idItr = vItemDataList.begin(); idItr != vItemDataList.end(); idItr++)
        {
            ...
            if ((*idItr).lItemType == ZT_MAIL)
            {
                ...
                printf(
                    ...
                    msgdata.deliveryDate,
                    msgdata.DeliveryDateString.c_str(), msgdata.HasAttachments,
                    msgdata.HasHtml, msgdata.HasText, msgdata.IsDraft, msgdata.IsFlagged,
                    msgdata.IsForwared,
                    msgdata.IsFromMe, msgdata.IsUnread, msgdata.IsUnsent, msgdata.RepliedTo,
                    msgdata.Urlname.c_str()
                    );
                if (msgdata.HasText)
