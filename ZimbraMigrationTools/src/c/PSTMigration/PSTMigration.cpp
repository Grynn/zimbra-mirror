...
BOOL FileUpload(Zimbra::Rpc::ZimbraConnection *z_connection, LPWSTR *ppwszToken) {
    ...
    HANDLE hFile =
        CreateFile(L"E:\\temp\\aa.log", GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
    ...
    BOOL bResult =
        z_connection->DoFileUpload(pStreamFile, dwFileSize, L"/service/upload?fmt=raw",
            ppwszToken,
            NULL);
...
void ZCFileUploadTest() {
    Zimbra::Rpc::UserSession *session = Zimbra::Rpc::UserSession::CreateInstance(
            lpProfileName, lpAccountUser, lpAccountUserPwd, lpServerAddress, nPort, 0, 0, L"",
            false);

    Zimbra::Rpc::UserSession::SetProfileName(lpProfileName);
    Zimbra::Rpc::ZimbraConnection *z_pConnection =
        new Zimbra::Rpc::ZimbraConnection(_T("Migration"), lpServerAddress, nPort,
            false, 0, L"");
 ...
 */
    ...
    Zimbra::MAPI::Util::GetUserDnAndServerDnFromProfile(
        p_zmmapisession->GetMAPISessionObject(), ExchangeServerDN, ExchangeUserDN);
    ...
    p_zmmapisession->OpenOtherStore(defaultStore.GetInternalMAPIStore(), pwstrExchangeServerDN,
        (LPWSTR)wstruserdn.c_str(), userStore);
