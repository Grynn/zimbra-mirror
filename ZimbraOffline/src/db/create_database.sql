...
CREATE TABLE IF NOT EXISTS ${DATABASE_NAME}.mail_item (
   ...
   blob_digest   VARCHAR(28) BINARY,         -- SHA-1 hash of blob, or NULL if item has no blob
...
CREATE TABLE IF NOT EXISTS ${DATABASE_NAME}.revision (
   ...
   CONSTRAINT fk_revision_item_id FOREIGN KEY (mailbox_id, item_id) REFERENCES ${DATABASE_NAME}.mail_item(mailbox_id, id) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS ${DATABASE_NAME}.open_conversation (
   ...
   CONSTRAINT fk_open_conversation_conv_id FOREIGN KEY (mailbox_id, conv_id) REFERENCES ${DATABASE_NAME}.mail_item(mailbox_id, id) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS ${DATABASE_NAME}.appointment (
   ...
   UNIQUE INDEX i_item_id (mailbox_id, item_id),
   CONSTRAINT fk_appointment_mailbox_id FOREIGN KEY (mailbox_id) REFERENCES zimbra.mailbox(id),
   CONSTRAINT fk_appointment_item_id FOREIGN KEY (mailbox_id, item_id) REFERENCES ${DATABASE_NAME}.mail_item(mailbox_id, id) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB;
...
-- Tracks UIDs of messages on remote POP3 servers
CREATE TABLE IF NOT EXISTS ${DATABASE_NAME}.pop3_message (
   ...
   UNIQUE INDEX i_uid_pop3_id (uid, data_source_id),
