...
public class GalSyncSAXHandler implements ElementHandler {
    ...
    public GalSyncSAXHandler(ZcsMailbox mainMbox, OfflineAccount galAccount,
            boolean fullSync, boolean trace) throws ServiceException {
        this.mainMbox = mainMbox;
        this.galAccount = galAccount;
        this.fullSync = fullSync;
        prov = OfflineProvisioning.getOfflineInstance();
        galMbox = MailboxManager.getInstance().getMailboxByAccountId(
                galAccount.getId(), false);
        context = new OperationContext(galMbox);

        ds = GalSyncUtil.createDataSourceForAccount(galAccount);

        syncFolder = OfflineGal.getSyncFolder(galMbox, context, fullSync)
                .getId();
        OfflineLog.offline.debug("Offline GAL current sync folder: "
                + Integer.toString(syncFolder));
    }
    ...
    public int getGroupCount() {
        return idGroups == null ? 0 : idGroups.size();
    }

    public String removeGroup() {
        return idGroups.remove(0);
    }
    ...
    public void onStart(ElementPath elPath) { // TODO: add trace logging;
        ...
        if (token == null) {
            OfflineLog.offline
                    .debug("Offline GAL parse error: SyncGalResponse has no token attribute");
        ...
        try {
            if (fullSync) {
                OfflineLog.offline.debug("Offline GAL full sync requested: "
                        + galAccount.getName());
    ...
    public void onEnd(ElementPath elPath) { // TODO: add trace logging;
        ...
        if (token == null) {
            OfflineLog.offline
                    .debug("Offline GAL parse error: missing SyncGalResponse tag");
            unregisterHandlers(elPath);
            return;
        }
        ...
        if (id == null) {
            OfflineLog.offline
                    .debug("Offline GAL parse error: cn has no id attribute");
            ...
            if (itr.hasNext()) {
                ...
                try {
                    saveUnparsedContact(id, map, null, false);
                } catch (Exception e) {
                    handleException(e, elPath);
                }
            } else { // Ids Only
    ...
    private void deleteContact(String id) throws ServiceException, IOException {
        ...
        if (iid > 0) {
            // always delete mapping first, so that in case of crash, unmapped
            // contacts can be cleaned up in runMaintenance()
            DbDataSource.deleteMapping(ds, iid);
            galMbox.delete(context, iid, MailItem.Type.CONTACT);
            OfflineLog.offline.debug("Offline GAL contact deleted: "
                    + Integer.toString(iid) + ", " + id);
        }
    }

    private void buildContactAttrMap(String id, Map<String, String> map)
        ...
        if (type.equals(OfflineGal.CTYPE_GROUP)
                && mainMbox.getRemoteServerVersion().isAtLeast7xx()) {
    ...
    private void createContactAndMapping(ParsedContact contact, String id, String logstr) throws ServiceException {
    ...
    private void saveContacts(List<ParsedContact> parsedContacts, List<String> strIds, List<Integer> ids) throws ServiceException, IOException {
        assert (parsedContacts.size() == strIds.size());
        int index = 0;
        for (ParsedContact c : parsedContacts) {
            try {
                saveParsedContact(strIds.get(index++), c, getContactLogStr(c), ids);
            } catch (MailServiceException e) {
                if (!MailServiceException.CONTACT_TOO_BIG.equals(e.getCode())) {
                    OfflineLog.offline
                            .debug("galsync exception when fetchContact " + e);
                    throw e;
                }
            }
        }
    ...
    private void saveMappings(List<Integer> ids, List<String> strIds) throws ServiceException {
        assert (ids.size() == ids.size());
        int index = 0;
        for (Integer id : ids) {
            DbDataSource.addMapping(ds, new DataSourceItem(0, id, strIds.get(index++), null), true);
        }
    }

    private void fillContactAttrMap(String id, Map<String, String> map)
            throws ServiceException {
        String fullName = map.get(ContactConstants.A_fullName);
        if (fullName == null) {
            String fname = map.get(ContactConstants.A_firstName);
    ...
    private void createContact(ParsedContact contact, String id, String logstr, List<Integer> ids) throws ServiceException {
        Contact c = galMbox.createContact(context, contact, syncFolder, null, true);
        ids.add(c.getId());
        OfflineLog.offline.debug("Offline GAL contact created: " + logstr);
    }

    private String getContactLogStr(ParsedContact contact) {
        StringBuilder logBuf = new StringBuilder();
        logBuf.append(" name=\"")
                .append(contact.getFields().get(ContactConstants.A_fullName))
                .append("\"").append(" type=\"")
                .append(contact.getFields().get(ContactConstants.A_type))
                .append("\"");
        return logBuf.toString();
    }

    private void saveParsedContact(String id, ParsedContact contact, String logstr, List<Integer> ids) throws ServiceException {
        if (fullSync) {
            createContact(contact, id, logstr, ids);
        } else {
            int itemId = GalSyncUtil.findContact(id, ds);
            if (itemId > 0) {
                try {
                    galMbox.modifyContact(context, itemId, contact, true);
                    OfflineLog.offline.debug("Offline GAL contact modified: "
                            + logstr);
                } catch (MailServiceException.NoSuchItemException e) {
                    OfflineLog.offline
                            .warn("Offline GAL modify error - no such contact: "
                                    + logstr
                                    + " itemId="
    ...
    private void saveUnparsedContact(String id, Map<String, String> map,
            List<Integer> ids, final boolean isBatch) throws ServiceException,
            IOException {
        buildContactAttrMap(id, map);
        ParsedContact contact = new ParsedContact(map);
        String logstr = getContactLogStr(contact);
        if (fullSync) {
            createContactAndMapping(contact, id, logstr);
        } else {
            int itemId = GalSyncUtil.findContact(id, ds);
            if (itemId > 0) {
                try {
                    galMbox.modifyContact(context, itemId, contact, isBatch);
                    OfflineLog.offline.debug("Offline GAL contact modified: "
                            + logstr);
                } catch (MailServiceException.NoSuchItemException e) {
                    OfflineLog.offline
    ...
    private void removeUnmapped() throws ServiceException {
        ...
        folderIds
                .add(OfflineGal.getSyncFolder(galMbox, context, false).getId());
        ...
        int sz = dsItems.size();
        if (sz < galItemIds.size()) { // proceed only if mapping size is less
                                      // than number of gal entries
            ...
            try {
                ...
                OfflineLog.offline
                        .warn("Offline GAL error in calculating set difference: "
                                + e.getMessage());
                return;
            }

            if (galItemIds.size() > 100) {
                prov.setAccountAttribute(mainMbox.getOfflineAccount(),
                        OfflineConstants.A_offlineGalAccountSyncToken, "");
                OfflineLog.offline.warn("Offline GAL too many unmapped items: "
                        + Integer.toString(galItemIds.size())
                        + ", falling back to full sync.");
            } else {
                for (Integer id : galItemIds) {
                    galMbox.delete(context, id.intValue(),
                            MailItem.Type.CONTACT);
                }
                OfflineLog.offline.debug("Offline GAL deleted "
                        + Integer.toString(galItemIds.size())
                        + " unmapped items.");
            }
        }
    ...
    // TODO change method signature when gal sync is totally fixed. no parameter
    // needed
    public long fetchContacts(long networkTime) throws ServiceException,
            IOException {
        XMLElement req = new XMLElement(MailConstants.GET_CONTACTS_REQUEST);
        req.addElement(AdminConstants.E_CN).addAttribute(AccountConstants.A_ID,
                removeGroup());
        long start = System.currentTimeMillis();
        Element response = mainMbox.sendRequest(req, true, true,
                OfflineLC.zdesktop_gal_sync_request_timeout.intValue(),
                SoapProtocol.Soap12);
        ...
        List<ParsedContact> parsedContacts = getParsedContacts(contacts, strIds);
        List<Integer> ids = new ArrayList<Integer>();
        
        boolean success = false;
        synchronized (this.galMbox) {
            try {
                this.galMbox.beginTransaction("GALSync", null);

                saveContacts(parsedContacts, strIds, ids);
                saveMappings(ids, strIds);

                success = true;
            } finally {
                this.galMbox.endTransaction(success);
            }
        }
        return networkTime;
    }

    private List<ParsedContact> getParsedContacts(List<Element> contacts, List<String> ids) throws ServiceException {
    ...
    public void runMaintenance() {
        long lastRefresh = galAccount.getLongAttr(
                OfflineConstants.A_offlineGalAccountLastRefresh, 0);
        long interval = OfflineLC.zdesktop_gal_refresh_interval_days
                .longValue();
        if (lastRefresh > 0
                && (System.currentTimeMillis() - lastRefresh)
                        / Constants.MILLIS_PER_DAY < interval)
            return;
        ...
        try {
            ...
            prov.setAccountAttribute(galAccount,
                    OfflineConstants.A_offlineGalAccountLastRefresh,
                    Long.toString(System.currentTimeMillis()));
        } catch (ServiceException e) {
            OfflineLog.offline.warn("Offline GAL maintenance error: "
                    + e.getMessage());
        }
    }
}
