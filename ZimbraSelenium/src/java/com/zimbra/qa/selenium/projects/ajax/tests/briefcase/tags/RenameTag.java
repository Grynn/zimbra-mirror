...
public class RenameTag extends AjaxCommonTest {
	...
	public void RenameTag_01() throws HarnessException {
		...
		DialogRenameTag dialog = (DialogRenameTag) app.zTreeBriefcase.zTreeItem(
				Action.A_RIGHTCLICK, Button.B_TREE_RENAMETAG, tag);
		...
		Element[] eTag1 = account.soapSelectNodes(
				"//mail:tag[@name='" + name1 + "']");
		ZAssert.assertEquals(eTag1.length, 0,
				"Verify the old tag name no longer exists");

		Element[] eTag2 = app.zGetActiveAccount().soapSelectNodes(
				"//mail:tag[@name='" + name2 + "']");
