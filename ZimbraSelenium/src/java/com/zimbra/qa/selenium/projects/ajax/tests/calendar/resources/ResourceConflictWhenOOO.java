...
public class ResourceConflictWhenOOO extends CalendarWorkWeekTest {	
	...
	@Test(description = "Verify if OOO status of Location causes Dobule booking",
			groups = { "functional" })
	...
	@Test(description = "Verify if OOO status of Equipment causes double booking",
			groups = { "functional" })
	public void EquipmentConflictWhenOOO_01() throws HarnessException {
		...
		app.zGetActiveAccount().soapSend(
                "<CreateAppointmentRequest xmlns='urn:zimbraMail'>" +
                     "<m>"+
                     	"<inv method='REQUEST' type='event' status='CONF' draft='0' class='PUB' fb='O' transp='O' allDay='0' name='"+ apptSubject1 +"'>" +
                     	"<s d='"+ startUTC.toTimeZone(tz).toYYYYMMDDTHHMMSS() +"' tz='"+ tz +"'/>" +
                 		"<e d='"+ endUTC.toTimeZone(tz).toYYYYMMDDTHHMMSS() +"' tz='"+ tz +"'/>" +
                     		"<or a='"+ app.zGetActiveAccount().EmailAddress +"'/>" +
                     		"<at role='REQ' ptst='NE' rsvp='1' a='" + apptAttendeeEmail + "' d='2'/>" +
                     		"<at cutype='RES' a='" + apptEquipment + "' rsvp='1' role='REQ' url='" + apptEquipment + "' ptst='NE' fb='O' fba='O'/>" +
                     	"</inv>" +
		...
		AppointmentItem actual = AppointmentItem.importFromSOAP(app.zGetActiveAccount(), "subject:("+ apptSubject2 +")");
		ZAssert.assertEquals(actual.getSubject(), apptSubject2, "Subject: Verify the appointment data");
		ZAssert.assertEquals(appt.getEquipment(), apptEquipment, "Equipment: Verify the location is present in the appointment");
	
		// Verify Equipment free/busy status shows as psts=DE
		String EquipmentStatus2 = ZimbraAccount.AccountA().soapSelectValue("//mail:at[@a='"+ apptEquipment +"']", "ptst");
		ZAssert.assertEquals(EquipmentStatus2, "DE", "Verify that the Equipment status shows as 'DECLINED'");
