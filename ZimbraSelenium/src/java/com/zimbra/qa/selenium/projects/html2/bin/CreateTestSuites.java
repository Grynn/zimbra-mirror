package com.zimbra.qa.selenium.projects.html2.bin;
...
public class CreateTestSuites {
	...
	private static String priorityLocales = "";
	private static String priorityBrowsers = "";
	private static String priorityClient = "qa33-vista-qtp2";
	...
	private static String[] clientsToUse = {"qafe1", "qafe2",
			 "browserperf1",
			"browserperf2", "qa33-vista-qtp1","qa33-vista-qtp2"};
	...
	private static String currentClientName = "";//store current client name
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			while ((str = br.readLine()) != null) {
				...
				if(!job.equals("")) {
					if(priorityLocales.indexOf(locale) >=0 || priorityBrowsers.indexOf(browser) >=0 || priorityClient.indexOf(currentClientName) >=0 )
	...
	public static String getJobIfMatched(String locale, String browser, String os) throws Exception {
		...
		if (foundBrowser) {
			...
			int num = generator.nextInt(matchedClients.size());
			 currentClientName = matchedClients.get(num);
			...
			String printStr = "/opt/qa/tools/kickoffTest.rb " + currentClientName
					+ " " + branch + " " + sMachine.osType
					+ " 'Selng&cmachine=" + currentClientName + "&s_locale=" + locale
