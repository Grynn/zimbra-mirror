...
//
	...
	protected AbsPage zTreeItem(Action action, FolderItem folder) throws HarnessException {
		...
		if ( action == Action.A_LEFTCLICK ) {
			...
			locator = "css=div[id='ext-simplelistitem-" + (Integer.parseInt(folder.getId())-1) + "'] span[class='x-list-item-leaf']";
	...
	public boolean zIsActive() throws HarnessException {
		...
		if ( !((AppTouchClient)MyApplication).zPageMail.zIsActive() ) {
			((AppTouchClient)MyApplication).zPageMail.zNavigateTo();
		}
