...
Logger.prototype = {
	...
	perfLogLevelCount: 0,//Prints info-level logs only when perfLogLevelCount == 2

	perfStartLogging: false,//this has to be true for setPrefLevelCount to work(is set to true by Selenium.prototype.doLogPerfMessage

	setPerfLogLevelCount: function(val) {
		if(this.perfStartLogging)
			this.perfLogLevelCount = val;

    },
	getPerfLogLevelCount: function() {
       return this.perfLogLevelCount;
    },
    ...
    openLogWindow: function() {
		...
		//throws error in Safari when popup is blocked
		try{
        this.logWindow.moveTo(window.screenX + 1210, window.screenY + window.outerHeight - 1400);
		} catch(e) {
			 throw new SeleniumError("Popup window was blocked by Browser");
		}
    ...
    log: function(logLevel, message) {
		...
		//if treshold is perfinfo,then, log only perfinfo
        if (this.threshold == "perfinfo") {
			if (logLevel != "perfinfo")
				return;
    ...
    perfinfo: function(message) {
       this.log("perfinfo", message);
    },
