...
public class UploadFile extends AjaxCommonTest {
	...
	@Test(	description = "Upload file through SOAP - verify through SOAP",	
			groups = { "sanity" })
	public void UploadFile_01() throws HarnessException {
		...
		document.createUsingSOAP(account, attachmentId);
	...
	@Test(	description = "Upload file through SOAP - verify through GUI", 
		groups = { "sanity" })
	public void UploadFile_02() throws HarnessException {
		...
		if(ClientSessionFactory.session().selenium().isElementPresent("css=[id='zl__BDLV__rows']")&&
			...
			name = ClientSessionFactory.session().selenium().getText("css=div[id='zl__BDLV__rows'][class='DwtListView-Rows'] td[width='auto'] div:contains(" + fileName + ")");
 		}
