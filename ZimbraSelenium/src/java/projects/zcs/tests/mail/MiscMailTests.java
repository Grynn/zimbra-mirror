...
public class MiscMailTests extends CommonTest {
	...
	public Object[][] createData(Method method) throws ServiceException {
		...
		} else if (test.equals("deepLinkTest")) {
			return new Object[][] { { selfAccountName, "ccuser@testdomain.com",
	...
	@Test(dataProvider = "mailDataProvider", groups = { "smoke", "full" }, retryAnalyzer = RetryFailedTests.class)
	public void deepLinkTest(String to, String cc, String bcc, String subject,
	...
	public void msgBodyLostWhileChooseNotToAttachSig_Bug40559(String to,
		...
		page.zAccPref
				.zcreateSignature("testSignature", "signatureBody", "TEXT");
		obj.zButton.zClick("id=zb__PREF__SAVE_left_icon");
		Thread.sleep(2000);
		zGoToApplication("Preferences");
		zGoToPreferences("Accounts");
	...
	/**
	 * Test Case:-Message is automatically selected(checkMsgStatusOfSelectedMsgWhileMovingOneFolderToOther)
	 * 1.Compose Mail to self
	 * 2.Go To Inbox
	 * 3.Click on Get Mail until it show up in Inbox.
	 * 4.Verify Msg In Reading Pane.
	 * 5.It should disply "To view a message, click on it" 
	 * 6.Go To another folder and check for the same msg "To view a message, click on it"
	 * 7. Means in short It should not display the content of the mail while going from one folder to another.OR
	...
	public void dontSelectFirstMsgByDefault_Bug39908_Bug43335(String to,
		...
		Assert.assertTrue(selenium.isElementPresent("xpath=//div[@id='zv__CLV__MSG']/table/tbody/tr/td"),"To view a message, click on it.Msg does not present");
		String BodyText=selenium.getText("xpath=//div[@id='zv__CLV__MSG']/table/tbody/tr/td");
		Assert.assertTrue(BodyText.contains(localize(locator.viewMessage)));
		obj.zFolder.zClick(localize(locator.sent));
		Assert.assertTrue(selenium.getText("xpath=//div[@id='zv__CLV__MSG']/table/tbody/tr/td").contains(localize(locator.viewMessage)));
		
		needReset = false;
	}
	/**
	 * Test Case:User cannot save preferences due to empty signature field
	 * Steps to replicate
	 * Login as user to Ajax webclient (as user with no existing sigs)
	 * Click Preferences
	 * Click Signatures
	 * Click another pref section, eg Instant messaging
	 * Change something and click save
	 * "Signature value is empty. It's required." appears at the top.
	 * Validation(Expected) above msg should not appear while saving empty signature field
	 * @param to
	 * @param cc
	 * @param bcc
	 * @param subject
	 ...
	 */
	@Test(dataProvider = "mailDataProvider", groups = { "smoke", "full" }, retryAnalyzer = RetryFailedTests.class)
	public void UserCanAbleToSavePrefWithEmptySignatureField_Bug44607(String to,
			String cc, String bcc, String subject, String body,
			String attachments) throws Exception {
		if (isExecutionARetry)
			handleRetry();
	...
	public void emptyTrash_Bug41209() throws Exception {
		...
		/**
		 * 1. Check deleted message is present in Trash.
		...
		obj.zButton.zClickInDlgByName(localize(locator.ok),localize(locator.warningMsg));

		/**
		 * 1. Go To Trash.
		 * 2. Check message is deleted from Trash.
		 */
		obj.zFolder.zClick(localize(locator.trash));
