...
public class ComposeReplyFwdInPlainTextTests extends CommonTest {
	...
	private Object[][] createData(Method method) {
		...
		if (test.equals("sendMailToSelfAndVerify")
			...
			return new Object[][] {
				...
				|| (test.equals("detachRetainsAttachments"))) {// use this for
			return new Object[][] { { "_selfAccountName_", "", "",
					getLocalizedData(2), "", "putty.log,samlejpg.JPG" }
		...
		} else if (test.equals("fwdingAMailWithAttachment_bug46375")) {
			return new Object[][] { { "_selfAccountName_", "", "",
	...
	// Tests..------------------------------------------------------
	/**
	 * Test: Send an email(to self) in text-mode in several
	 * ways(to-only,cc-only, etc) and verify if the received mail has all the
	 * information
	 */
	@Test(dataProvider = "composeDataProvider", groups = { "smoke", "full" }, retryAnalyzer = RetryFailedTests.class)
	public void sendMailToSelfAndVerify(String to, String cc, String bcc,
	...
	/**
	 * Test: Send an email(to self) in text-mode and in newwindow in several
	 * ways(to-only,cc-only, etc) and verify if the received mail has all the
	 * information
	 */
	@Test(dataProvider = "composeDataProvider", groups = { "smoke", "full" }, retryAnalyzer = RetryFailedTests.class)
	public void sendMailToSelfAndVerify_NewWindow(String to, String cc,
	...
	// tests...----------------------------------------
	/**
	 * Test: Reply to an email in plain-mode and verify if the mail-compose and
	 * verify that cc and bcc is empty,to is filled, subject has Re appended and
	 * body is intact
	 */
	@Test(dataProvider = "composeDataProvider", groups = { "smoke", "full" }, retryAnalyzer = RetryFailedTests.class)
	public void replyTest(String to, String cc, String bcc, String subject,
	...
	/**
	 * Test: Reply to an email in plain-mode and in new-window and verify if the
	 * mail-compose and verify that cc and bcc is empty,to is filled, subject
	 * has Re appended and body is intact
	 */
	@Test(dataProvider = "composeDataProvider", groups = { "smoke", "full" }, retryAnalyzer = RetryFailedTests.class)
	public void replyTest_NewWindow(String to, String cc, String bcc,
			String subject, String body, String attachments) throws Exception {
		if (SelNGBase.isExecutionARetry.get())
			handleRetry();
		page.zComposeView.zNavigateToComposeByShiftClick();
	...
	// ----------------------Reply All tests...------------------
	/**
	 * Test: "Reply-all" to an email in plain-mode and verify if the
	 * mail-compose and verify that cc, bcc,to,subject and body are filled,
	 * subject has Re prepended and body is intact
	 */
	@Test(dataProvider = "composeDataProvider", groups = { "smoke", "full" }, retryAnalyzer = RetryFailedTests.class)
	public void replyAllTest(String to, String cc, String bcc, String subject,
			String body, String attachments) throws Exception {
		if (SelNGBase.isExecutionARetry.get())
			handleRetry();
		page.zComposeView.zNavigateToMailCompose();
	...
	/**
	 * Test: "Reply-all" to an email in plain-mode and in-newwindow and verify
	 * if the mail-compose and verify that cc, bcc,to,subject and body are
	 * filled, subject has Re prepended and body is intact
	 */
	@Test(dataProvider = "composeDataProvider", groups = { "smoke", "full",
			"test24" }, retryAnalyzer = RetryFailedTests.class)
	public void replyAllTest_NewWindow(String to, String cc, String bcc,
			String subject, String body, String attachments) throws Exception {
		if (SelNGBase.isExecutionARetry.get())
			handleRetry();
		page.zComposeView.zNavigateToComposeByShiftClick();
	...
	// -------------------Forward tests...----------------------------
	/**
	 * Test: Hit "Forward" to an email in plain-mode and verify if the
	 * mail-compose and verify that cc, bcc and to are empty, but subject and
	 * body are filled. Also: subject has Fwd prepended
	 */
	@Test(dataProvider = "composeDataProvider", groups = { "smoke", "full" }, retryAnalyzer = RetryFailedTests.class)
	public void fwdMailTest(String to, String cc, String bcc, String subject,
			String body, String attachments) throws Exception {
		if (SelNGBase.isExecutionARetry.get())
			handleRetry();
		page.zComposeView.zNavigateToMailCompose();
	 ...
	 */
	@Test(dataProvider = "composeDataProvider", groups = { "smoke", "full" }, retryAnalyzer = RetryFailedTests.class)
	public void fwdMailTest_NewWindow(String to, String cc, String bcc,
			String subject, String body, String attachments) throws Exception {
	 ...
	 * 
	 * @Steps:- Login to web client Assume you have conversation which has lot
	 *          of reply Double click to it Again double click to message so it
	 *          switches its view to message view with previous/next buttons
	 *          Press Next button to read mails one by one
	 * 
	 *          each time it should not composes new mail and should not shows
	 *          in separate tab.
	 * @param to
	 * @param cc
	 * @param bcc
	...
	@Test(dataProvider = "composeDataProvider", groups = { "smoke", "full" }, retryAnalyzer = RetryFailedTests.class)
	public void checkMesgViewWithInTabWithArrowPosition_40614(String to,
		...
		Assert.assertTrue(SelNGBase.selenium.get().isElementPresent("id=zv__CV_closeBtnCell"));
		/*
		...
		Assert
				.assertTrue(
						SelNGBase.selenium.get()
								.isElementPresent("xpath=//td[contains(@id,'zb__MSG1__Nav__PAGE_BACK_left_icon')]/div[contains(@class,'ImgLeftArrow ZDisabledImage')]"),
						"Left Arrow displyed as enable");
		Assert
				.assertTrue(
						SelNGBase.selenium.get()
								.isElementPresent("xpath=//td[contains(@id,'zb__MSG1__Nav__PAGE_FORWARD_left_icon')]/div[contains(@class,'ImgRightArrow')]"),
						"Right Arrow displyed as disable");
		Assert
				.assertTrue(
						SelNGBase.selenium.get()
								.isElementPresent("xpath=//td[contains(@id,'zb__App__DWT') and contains(text(),'Re:')]"),
						"Tab is not showing Re:");
		SelNGBase.selenium.get()
				.mouseOver("//td[contains(@id,'zb__MSG1__Nav__PAGE_FORWARD_left_icon')]/div[contains(@class,'ImgRightArrow')]");
		SelNGBase.selenium.get()
				.clickAt(
						"//td[contains(@id,'zb__MSG1__Nav__PAGE_FORWARD_left_icon')]/div[contains(@class,'ImgRightArrow')]",
						"");
		Assert
				.assertTrue(
						SelNGBase.selenium.get()
								.isElementPresent("xpath=//td[contains(@id,'zb__App__DWT') and contains(text(),'Re:')]"),
						"After clicking Right arrow Tab is not showing Re:");

		SelNGBase.needReset.set(false);
	}

	/**
	 * Test Case:-Issue with forwarding email as attachment Set Forwarding
	 * preference to "Include original message as an attachment" (Preferences
	 * --> Mail --> Composing) Forward a message (original shows as attachment)
	 * Click on Options in the "New Message" tool-bar and change from "Include
	 * Original As Attachment" to "Include Original Message" Expected:-
	 * Attachment msg should get convert into check box attachment with some
	 * text in body.
	 * 
	 * @param to
	 * @param cc
	...
	@Test(dataProvider = "composeDataProvider", groups = { "smoke", "full" }, retryAnalyzer = RetryFailedTests.class)
	public void fwdingAMailWithAttachment_bug46375(String to, String cc,
			String bcc, String subject, String body, String attachments)
			throws Exception {
		if (SelNGBase.isExecutionARetry.get())
			handleRetry();
		 ...
		 * doesn't work then we can use following xpath--
		 */
		/*
		 ...
		 * "xpath=//html/body/div[4]/div[16]/div[2]/div[3]/table[2]/tbody/tr/td/table/tbody/tr[12]/td[2]/table/tbody/tr/td/div/table/tbody/tr/td/table/tbody/tr/td[4]/div"
		 * ,"");selenium.mouseUpAt(
		 * "xpath=//html/body/div[4]/div[16]/div[2]/div[3]/table[2]/tbody/tr/td/table/tbody/tr[12]/td[2]/table/tbody/tr/td/div/table/tbody/tr/td/table/tbody/tr/td[4]/div"
		 * ,"");
		 */
