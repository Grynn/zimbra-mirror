...
ZaAccount.A_zimbraYahooId = "zimbraYahooId" ;
	...
	**/
	...
	if(tmpObj.attrs[ZaAccount.A_zimbraMailMessageLifetime] != "" && tmpObj.attrs[ZaAccount.A_zimbraMailMessageLifetime] !=null) {
		...
		if(itestVal > 0 && itestVal < 31) {
			app.getCurrentController().popupErrorDialog(ZaMsg.ERROR_MESSAGE_LIFETIME_BELOW_31);
			return false;
		}
	}			
...
function (tmpObj, account, app) {
	...
	var resp;	
	//create SOAP request
	var soapDoc = AjxSoapDoc.create("CreateSmbAccountRequest", ZaZimbraAdmin.URN, null);
    ...
    /*** PA - we must set the cosId to  starter
    if (tmpObj[ZaAccount.A2_autoCos] == "TRUE" ) {
		tmpObj.attrs[ZaAccount.A_COSId] = null ;
	} */

    //always starter type for the PA user
    tmpObj.attrs[ZaAccount.A_COSId] =  app.getCosIdByCosName(SMBAccount.getCosValueByAccountType(SMBAccount.TYPE_STARTER));

    //always set the account status to suspended state
    tmpObj.attrs[ZaAccount.A_accountStatus] = SMBAccount.SUSPENDED_STATUS ;
    ...
    //set yahoo specific attributes
    SMBAccount.setYahooAttrsValue(tmpObj.yahoo, soapDoc) ;
    ...
    try {
		...
		resp = ZaRequestMgr.invoke(csfeParams, reqMgrParams ).Body.CreateSmbAccountResponse;
...
function(mods) {
	...
	resp = ZaRequestMgr.invoke(params, reqMgrParams).Body.ModifySmbAccountResponse ;
...
ZaAccount.myXModel = { 
	items: [
        ...
        {id:ZaAccount.A_zimbraYahooId, type:_STRING_, ref:"attrs/"+ZaAccount.A_zimbraYahooId},
...
function (value, event, form){
    ...
    //for PA domain admin, the domain name will never changed
    ...
    try {
		...
		if ((!ZaSettings.COSES_ENABLED) ||  ZaSettings.isYahooSmbDomainAdmin){
		...
		}else if ((ZaSettings.COSES_ENABLED) && (! form.parent._isCosChanged)
        ...
        if (ZaDomain.A_domainMaxAccounts && p._domains && p._domains[newDomainName]){
			...
			if (maxDomainAccounts && maxDomainAccounts > 0) {
			    ...
			    instance[ZaAccount.A2_domainUsedAccounts] =
                    AjxMessageFormat.format(com_zimbra_yahoosmb.AccountsAvailable, [usedAccounts, maxDomainAccounts - usedAccounts]);
            }else{
				instance[ZaAccount.A2_domainLeftAccounts] = null ;
                instance[ZaAccount.A2_domainUsedAccounts] = null ;
            }
		}       */
