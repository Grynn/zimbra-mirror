...
function () {
	if(this._localXForm.hasErrors()) {
		...
		if(this._containedObject.attrs[ZaAccount.A_lastName]
                ...
                && SMBAccount.checkValue(this._containedObject.yahoo)) {
...
function() {
	try {
        ...
        //ideally, the yahoo attribute should also be returned from server. However, we currently don't save the attribute values on the server side.
        account.yahoo = this._containedObject.yahoo ;
        if(account != null) {
			//if creation took place - fire an change event
			this._app.getAccountListController().fireCreationEvent(account);
			this.popdown();

            //present the confirmation Dialog to the admin
            if(!this._app.dialogs["createUserConfirmationDialog"]) {
                this._app.dialogs["createUserConfirmationDialog"] =
                    new PACreateUserConfirmDialog (this._app._container, this._app, "480px", "150px");
            }
            //this._app.dialogs["moveAliasDialog"].setAlias(alias);
            this._app.dialogs["createUserConfirmationDialog"].popup(account);
        }
	} catch (ex) {
		switch(ex.code) {		
...
function(entry) {
    ...
    this._containedObject.attrs[ZaAccount.A_accountStatus] = SMBAccount.SUSPENDED_STATUS;
...
//	var domainName = this._app._appCtxt.getAppController().getOverviewPanelController().getCurrentDomain();
    ...
    if(ZaSettings.GLOBAL_CONFIG_ENABLED) {
        /** PA admin always use ZaSettings.myDomainName
        if(!domainName) {
    ...
    //Handle Yahoo SMB Attributes
    SMBAccount.setObject.call (this, entry) ;
...
ZaNewAccountXWizard.myXFormModifier = function(xFormObject) {	
	var domainName;
    if (ZaSettings.isYahooSmbDomainAdmin) {
        domainName = ZaSettings.myDomainName;
    }else  if(ZaSettings.DOMAINS_ENABLED && this._app.getDomainList().size() > 0)
		domainName = this._app.getDomainList().getArray()[0].name;
	else 
		domainName = ZaSettings.myDomainName;
	...
	var case1Items = [ 
		...
		{type:_ZAWIZ_TOP_GROUPER_, label:ZaMsg.NAD_AccountNameGrouper, id:"account_wiz_name_group",numCols:2,
			...
			{ref:ZaAccount.A_name, type:_EMAILADDR_, msgName:ZaMsg.NAD_AccountName,label:ZaMsg.NAD_AccountName,
                    ...
                    /**Disable for PA
            {ref: ZaAccount.Y_yahoo_id, type: _TEXTFIELD_, msgName:ZaAccount.Y_yahoo_id, label: com_zimbra_yahoosmb.Yahoo_ID, labelLocation:_LEFT_,
                onChange: SMBAccount.onYahooIdChanged }, */
	...
	var setupGroup = {type:_ZAWIZ_TOP_GROUPER_, label:ZaMsg.NAD_AccountSetupGrouper, id:"account_wiz_setup_group", 
		numCols:2,
		items: [
                /** Not needed for PA user , field read-only
            {ref:ZaAccount.A_accountStatus, type:_OSELECT1_, msgName:ZaMsg.NAD_AccountStatus,
				label:ZaMsg.NAD_AccountStatus,
                relevant: false, relevantBehavior: _DISABLE_,
                labelLocation:_LEFT_, choices:this.accountStatusChoices
			} ,   */

            {ref:ZaAccount.Y_notification_email, type:_TEXTFIELD_, label:com_zimbra_yahoosmb.NotificationEmail,
							 labelLocation:_LEFT_,forceUpdate:true , width: 200
			}    
        ]
	}

    /** Not needed for PA user
    if(ZaSettings.COSES_ENABLED) {
				...
				items: [
                    ...
                    {type:_ZAWIZ_TOP_GROUPER_, label:ZaMsg.NAD_zimbraMajorFeature, id:"account_wiz_features_major", colSizes:["auto"],numCols:1,
						items:[
                            ...
                            {ref:ZaAccount.A_zimbraFeatureImapDataSourceEnabled, type:_SUPER_WIZ_CHECKBOX_, resetToSuperLabel:ZaMsg.NAD_ResetToCOS,
                                msgName:ZaMsg.NAD_zimbraExternalImapEnabled,checkBoxLabel:ZaMsg.NAD_zimbraExternalImapEnabled,
                                trueValue:"TRUE", falseValue:"FALSE"},
