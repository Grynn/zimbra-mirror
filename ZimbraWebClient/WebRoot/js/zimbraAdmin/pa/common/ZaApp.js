...
function(appCtxt) {
    ...
    } else if(ZaSettings.DOMAINS_ENABLED) {
		this.searchDomains("");
	}
}
...
function(refresh) {
	if (refresh || this._domainList == null) {
        if (ZaSettings.isYahooSmbDomainAdmin) {
           this._domainList = SMBAccount.getSMBDomainList(this);
        } else {
...
ZaApp.prototype.getCosIdByCosName = function (cosName) {
   var cosArray = this.getCosList().getArray();
    var i = ZaUtil.findValueInObjArrByPropertyName(cosArray, cosName, "name") ;
    return cosArray [i].id ;
}
