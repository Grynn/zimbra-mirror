...
ZaCos.getAll =
function(app) {
	var soapDoc = AjxSoapDoc.create("GetAllSmbCosRequest", ZaZimbraAdmin.URN, null);	
	...
	var resp = ZaRequestMgr.invoke(params, reqMgrParams).Body.GetAllSmbCosResponse;
...
function (accountName, cosList, app){
	...
	for(var i in cosList.getIdHash()) {
		if(idHash[i].name == "starter") { //default cos name is starter.
			defaultCos = idHash[i];
		}
	}
