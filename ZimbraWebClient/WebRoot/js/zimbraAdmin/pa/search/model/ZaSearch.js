...
ZaSearch.getPredefinedSavedSearches =  function (app) {
    var starterCosId = app.getCosIdByCosName (SMBAccount.getCosValueByAccountType(SMBAccount.TYPE_STARTER));
    return [
        {name: com_zimbra_yahoosmb.ss_starter_accounts, query: "(zimbraCosId="+ starterCosId + ")"},
        {name: com_zimbra_yahoosmb.ss_suspended_accounts, query: "(zimbraAccountStatus=*" + SMBAccount.SUSPENDED_STATUS +  "*)"} ,
        {name: ZaMsg.ss_locked_out_accounts, query: "(zimbraAccountStatus=*lockout*)"},
        {name: ZaMsg.ss_closed_accounts, query: "(zimbraAccountStatus=*closed*)"},
        {name: ZaMsg.ss_maintenance_accounts, query: "(zimbraAccountStatus=*maintenance*)"},
        {name: ZaMsg.ss_non_active_accounts, query: "(!(zimbraAccountStatus=*active*))" },
...
function (app) {
	...
	if (! currentSavedSearches){//load the predefined searches
        ...
        var predefinedSearches = ZaSearch.getPredefinedSavedSearches(app) ;
        for (var n=0; n < predefinedSearches.length; n ++) {
			savedSearchArr.push (predefinedSearches[n]) ;
		}
