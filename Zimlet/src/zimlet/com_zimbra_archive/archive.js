...
ZmArchiveZimlet.prototype.initializeToolbar =
		function(app, toolbar, controller, viewId) {
			...
			if (viewId == ZmId.VIEW_CONVLIST || viewId == ZmId.VIEW_CONV || viewId == ZmId.VIEW_TRAD ||
				...
				toolbar.getButton(ZmOperation.DELETE).getHtmlElement().style.display = "none";

				var buttonIndex = -1;
				for (var i = 0, count = toolbar.opList.length; i < count; i++) {
					if (toolbar.opList[i] == ZmOperation.DELETE) {
						buttonIndex = i + 1;
						break;
					}
				}
				...
				if (!toolbar.getOp(ZmArchiveZimlet.ARCHIVE_BUTTON_ID)) {
					...
					var listView = controller.getCurrentView() || controller.listView || controller._listView;
					if (listView) {
...
ZmArchiveZimlet.prototype._getFolderHandler =
		function (params, response) {
			...
			if (respObj && respObj.GetFolderResponse && respObj.GetFolderResponse.folder) {
				var folders = respObj.GetFolderResponse.folder[0];
			}
			if (folders) {
