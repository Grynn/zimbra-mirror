<%@ page language="java" import="javax.mail.*,javax.mail.internet.*,java.util.*,javax.sip.*,javax.sip.address.*,javax.sip.header.*,javax.sip.message.*,java.security.MessageDigest,java.security.NoSuchAlgorithmException" %>
...
final class Invite implements SipListener {
	...
	public void processResponse(ResponseEvent responseReceivedEvent) {
		...
		try {
			if (response.getStatusCode() == Response.OK
				...
				inviteRequest.setHeader((ViaHeader)response.getHeader("Via"));
				inviteRequest.setHeader((ToHeader)response.getHeader("To"));
				inviteRequest.setHeader((CallIdHeader)response.getHeader("Call-ID"));
				inviteRequest.setHeader((FromHeader)response.getHeader("From"));
	...
	public String md5Digest(String s) {
		...
		for(int i=0;i<bin.length;i++) {
			result[j++] = hxArray[(bin[i]>>4)&0xf];
			result[j++] = hxArray[bin[i]&0xf];
		}
	...
	public void init(String args[]) {
		...
		try {
			...
			for (int forIndex = 0; forIndex < toArray.length; forIndex++) {
				...
				// create Request URI
				requestURI =
					addressFactory.createSipURI(toArray[forIndex], peerHostPort);
