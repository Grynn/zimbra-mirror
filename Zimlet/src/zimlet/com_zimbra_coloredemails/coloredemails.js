...
com_zimbra_coloredemails.prototype._showPreferenceDlg = function() {
    ...
    this._preferenceDialog = this._createDialog({title:"'Colored Emails' Zimlet Preferences", view:this._preferenceView, standardButtons:[DwtDialog.OK_BUTTON, DwtDialog.CANCEL_BUTTON]});
...
function() {
    ...
    html[i++] = "<BR>";
    html[i++] = "<TABLE  width=95%>";
    html[i++] = "<TR><TD><input id='turnONColoredEmailsChkbox'  type='checkbox'/>Turn ON 'Email Quotes'-Zimlet</TD></TR>";
...
function() {
    ...
    if (doc.style.display == "none") {
        ...
        this._preferenceView.setSize("520", "160");
...
function(showInfoMsg) {
    ...
    var val = this._mAddedEmailsTextarea.value;
    ...
    var nonDupeArry = [];
    for (var i = 0; i < arry.length; i++) {
        var val = AjxStringUtil.trim(arry[i]);
        if (val.indexOf(email) == -1 && val.indexOf("@") > 0 && val.indexOf(".") > 0) {
            nonDupeArry.push(val);
        }
    }
	...
	if(showInfoMsg)
		appCtxt.getAppController().setStatusMsg("New Color Added. Pressing OK would Save the Changes", ZmStatusView.LEVEL_INFO);
