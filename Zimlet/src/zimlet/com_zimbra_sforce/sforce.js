...
Com_Zimbra_SForce.prototype.init =
function() {
	// Let the framework know that we want to handle 500 and other errors
	// ourselves (for a simple "login failure" the SForce server likes to return 500.
	...
	if (ZmAssistant && ZmAssistant.register) {
		ZmAssistant.register(asst);
	}
};

Com_Zimbra_SForce.prototype.onShowView =
...
Com_Zimbra_SForce.prototype._initComposeSFToolbar =
function() {
...
Com_Zimbra_SForce.prototype._makeEnvelope =
function(method) {
	var soap = AjxSoapDoc.create(method, this.XMLNS, null, "http://schemas.xmlsoap.org/soap/envelope/");
	var envEl = soap.getDoc().firstChild;
	// Seems we need to set these or otherwise will get a "VersionMismatch" message from SForce
...
Com_Zimbra_SForce.prototype.xmlToObject =
function(result) {
	try {
...
Com_Zimbra_SForce.prototype.rpc =
function(soap, callback, passErrors) {
...
Com_Zimbra_SForce.prototype.login =
function(callback) {
	if (!callback) {
		callback = false;
	}
...
Com_Zimbra_SForce.prototype.done_login =
...
// There's no explicit logout command to SForce, we just clear session data and
// user information.
...
Com_Zimbra_SForce.prototype.query =
function(query, limit, callback) {
	if (!this.sessionId) {
		this.login(function() { this._doQuery(query, limit, callback); });
	} else {
		this._doQuery(query, limit, callback);
	}
};

Com_Zimbra_SForce.prototype._doQuery =
function(query, limit, callback) {
	if (!limit || limit < 1) {limit = 1;}
	var soap = this._makeEnvelope("query");
	soap.set("queryString", query);
	// SForce docs specify this is a *wish* value and that it's not enforced.
	// Also says the valid range is 200 -> 2000. Default is 500. So callers need
	// to handle getting more or less results than they request.
...
Com_Zimbra_SForce.__query_result_get =
function() {
...
Com_Zimbra_SForce.prototype.done_query =
function(callback, result) {
...
Com_Zimbra_SForce.prototype.createSFObject =
function(props, type, callback) {
...
Com_Zimbra_SForce.prototype._do_createSFObject =
function(props, type, callback) {
	if (!callback) {
		callback = false;
	}
...
Com_Zimbra_SForce.prototype.done_createSFObject =
function(callback, result) {
	var xd = this.xmlToObject(result);
	if (xd && callback) {
...
// contact.
Com_Zimbra_SForce.prototype.contactDropped =
function(contact) {
	// Note that since all communication is required to be asynchronous, the
	// only way we can write this function is using a series of callbacks.  The
	// main entry point is when we call this.query(...), but then execution will
	// vary depending on the response.
	var acct_Website = "";
	...
	if (contact.email || contact.email2 || contact.email3) {
		...
		// clearly we can't search for a matching account, so let's create one
		this.dlg_createAccount({ get: Com_Zimbra_SForce.__query_result_get }, contact);
	}
};

Com_Zimbra_SForce.prototype.dlg_createAccount =
function(acct_data, contact_data) {
	...
	// We don't really want to mess with things like cache-ing this dialog...
	if (!contact_data._exists) {
		dlg.setButtonListener(
...
Com_Zimbra_SForce.prototype.noteDropped =
function(note) {
	if (!note) { return; }
	...
	function addEmails(a) {
		if (a) {
			if (typeof a == "string") {
				if (a.indexOf(ignoreDomain) != -1) { return; }
				emails.push(a);
	...
	function $search_acct(records) {
		...
		for (var i = 0; i < records.length; ++i) {
			records[i].TYPE = "O";
			acctsSorted[records[i].AccountId].Opp.push(records[i]);
		}
	...
	if (domains.length == 0) {
		...
		var q = ["select Id, FirstName, LastName, Email, AccountId from Contact where Email like '%",
				domains.join("%' or Email like '%"), "%'" ].join("");
		this.query(q, 10, $search_contact);
	}
};

Com_Zimbra_SForce.prototype.dlg_addNoteToAccounts =
function(accounts, note) {
	var view = new DwtComposite(this.getShell());
...
Com_Zimbra_SForce.toIsoDate =
function(theDate) {
	return AjxDateFormat.format("yyyy-MM-dd", theDate);
};

Com_Zimbra_SForce.toIsoDateTime =
function(theDate) {
...
Com_Zimbra_SForce.prototype.apptDropped =
function(obj) {
	var appt = {
...
Com_Zimbra_SForce.prototype.doDrop =
function(obj) {
	switch (obj.TYPE) {
		case "ZmMailMsg":
		case "ZmConv":		this.noteDropped(obj); break;
		case "ZmContact":	this.contactDropped(obj); break;
		case "ZmAppt":		this.apptDropped(obj); break;
		default:			this.displayErrorMessage("You somehow managed to drop a \"" +
													 obj.TYPE +
...
Com_Zimbra_SForce.prototype.singleClicked =
function() {
	this.login();
};
...
Com_Zimbra_SForce.prototype.menuItemSelected =
function(itemId) {
	switch (itemId) {
...
Com_Zimbra_SForce.prototype._sendAddSForce =
function(ev) {
...
Com_Zimbra_SForce.prototype._checkBoxHtml =
function(rec, cbid, indent, checked, html) {
